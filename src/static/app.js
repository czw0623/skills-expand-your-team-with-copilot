document.addEventListener("DOMContentLoaded", () => {
    ...
    Object.entries(allActivities).forEach(([name, details]) => {
      ...
      if (currentDifficulty !== "") {
        if (currentDifficulty === "all") {
          // Show only activities with no difficulty specified (suitable for all levels)
          if (details.difficulty) {
            return;
          }
        } else {
          // Show only activities matching the selected difficulty level
  ...
  difficultyFilters.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (currentDifficulty === selected) {
        // Clicking the active button again resets the difficulty filter
